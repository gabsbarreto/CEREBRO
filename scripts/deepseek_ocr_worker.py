...
class NoRepeatNGramLogitsProcessor:
    """Adapted from existing Translation Project DeepSeek OCR worker."""

    def __init__(self, ngram_size: int, window_size: int = 100, whitelist_token_ids: set[int] | None = None) -> None:
        self.ngram_size = max(int(ngram_size), 1)
...
def _looks_like_echoed_prompt(line: str) -> bool:
    lowered = line.lower()
    return ("free" in lowered and "ocr" in lowered) or (
        "extract" in lowered and ("readable" in lowered or "text" in lowered)
    )
...
def load_deepseek_ocr(model_name: str):
    """Load DeepSeek-OCR-2 using the local MLX-VLM processor path.

    Adapted from the existing Translation Project worker because mlx-vlm's generic
    load helper does not reliably resolve the DeepSeek-OCR-2 processor in-process.
    """
    from mlx_vlm.models.deepseekocr_2.processing_deepseekocr import DeepseekOCR2Processor
    from mlx_vlm.utils import StoppingCriteria, get_model_path, load_image_processor, load_model, load_tokenizer
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run DeepSeekOCR2 over page images.")
    ...
    if not model_name:
        raise SystemExit("DeepSeekOCR2 model path/model is required.")
    ...
    emit({"event": "ocr_started", "total_pages": len(image_paths)})
    if int(args.batch_size) > 1:
        emit({"event": "ocr_warning", "message": "Batch OCR is not enabled for DeepSeekOCR2; processing pages one at a time."})
    emit({"event": "ocr_model_loading", "model": model_name})
    model, processor = load_deepseek_ocr(model_name)
    config = model.config
    logits_processors = [
    ...
    emit({"event": "ocr_model_loaded", "total_pages": len(image_paths)})
    try:
        for index, image_path in enumerate(image_paths, start=1):
            emit({"event": "ocr_page_started", "page": index, "total_pages": len(image_paths), "image": str(image_path)})
            generation_prompt = build_generation_prompt(processor, config, str(args.prompt), apply_chat_template)
            result = generate(
                model,
                processor,
                generation_prompt,
        ...
        emit({"event": "ocr_finished", "total_pages": len(image_paths)})
    finally:
        cleanup_mlx()
    return 0
